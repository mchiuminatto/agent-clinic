#!/usr/bin/env python3
"""Maintain CHANGELOG.md for agent-clinic from git commit history."""
...
def git_log(since_date=None):
    """Return commits touching this directory as {date: [subject, ...]} newest-first."""
    cmd = ["git", "log", "--format=%ad|%s", "--date=short", "--", "."]
    if since_date:
        cmd.insert(3, f"--after={since_date}")
...
def main():
    ...
    if not changelog.exists():
        by_date = git_log()
        if not by_date:
            print("No commits found for this directory — nothing to write.")
            sys.exit(0)
